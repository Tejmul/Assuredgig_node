...
router.get('/available-applications/', requireAuth, async (req, res, next) => {
  try {
...
router.get('/:uuid/', requireAuth, async (req, res, next) => {
  try {
...
router.get('/:uuid/history/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid() });
    const { uuid } = schema.parse(req.params);
    ...
    const history = await prisma.contractUpdateHistory.findMany({
...
router.post('/:uuid/accept/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid() });
    const { uuid } = schema.parse(req.params);
    ...
    if (req.user.id === contract.clientId) {
      ...
      await prisma.contract.update({ where: { id: contract.id }, data: { status: 'active', freelancerApprovedAt: new Date() } });
    }
...
router.post('/:uuid/reject/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid() });
    const { uuid } = schema.parse(req.params);
...
router.post('/:uuid/complete/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid() });
    const { uuid } = schema.parse(req.params);
...
// Draft endpoints (minimal; stored in ContractUpdateHistory.draftData/draftMilestones)
router.get('/:uuid/draft/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid() });
    const { uuid } = schema.parse(req.params);
...
router.post('/:uuid/create-draft/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid() });
    const { uuid } = schema.parse(req.params);
    const contract = await prisma.contract.findUnique({ where: { uuid } });
...
router.post('/:uuid/approve-draft/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid() });
    const { uuid } = schema.parse(req.params);
    let contract = await prisma.contract.findUnique({ where: { uuid } });
...
router.post('/:uuid/reject-draft/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid() });
    const { uuid } = schema.parse(req.params);
    const contract = await prisma.contract.findUnique({ where: { uuid } });
...
router.post('/:uuid/cancel-draft/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid() });
    const { uuid } = schema.parse(req.params);
    const contract = await prisma.contract.findUnique({ where: { uuid } });
...
// Milestones
router.get('/:uuid/milestones/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid() });
    const { uuid } = schema.parse(req.params);
    const contract = await prisma.contract.findUnique({ where: { uuid } });
...
router.post('/:uuid/milestones/create/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid() });
    const { uuid } = schema.parse(req.params);
    const contract = await prisma.contract.findUnique({ where: { uuid } });
...
router.get('/:uuid/milestones/:milestone_uuid/', requireAuth, async (req, res, next) => {
  try {
...
router.post('/:uuid/milestones/:milestone_uuid/complete/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid(), milestone_uuid: z.string().uuid() });
    const { uuid, milestone_uuid } = schema.parse(req.params);
...
router.post('/:uuid/milestones/:milestone_uuid/approve/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid(), milestone_uuid: z.string().uuid() });
    const { uuid, milestone_uuid } = schema.parse(req.params);
...
router.post('/:uuid/milestones/:milestone_uuid/reject/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid(), milestone_uuid: z.string().uuid() });
    const { uuid, milestone_uuid } = schema.parse(req.params);
...
// Disputes
router.get('/:uuid/disputes/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid() });
    const { uuid } = schema.parse(req.params);
    const contract = await prisma.contract.findUnique({ where: { uuid } });
...
router.post('/:uuid/disputes/create/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid() });
    const { uuid } = schema.parse(req.params);
    let contract = await prisma.contract.findUnique({ where: { uuid } });
...
router.get('/:uuid/disputes/:dispute_uuid/', requireAuth, async (req, res, next) => {
  try {
...
router.post('/:uuid/disputes/:dispute_uuid/close/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid(), dispute_uuid: z.string().uuid() });
    const { uuid, dispute_uuid } = schema.parse(req.params);
    const contract = await prisma.contract.findUnique({ where: { uuid } });
...
// Progress updates: enforce 3/day per contract
router.post('/:uuid/request-progress-update/', requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ uuid: z.string().uuid() });
    const { uuid } = schema.parse(req.params);
